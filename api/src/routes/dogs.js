...
router.get("/", async (req, res) => {
  try {
    ...
    if (name) {
      const filtered = allDogs.filter((el) => el.name.toLowerCase().includes(name.toLowerCase()));
...
router.post("/", async (req, res) => {
  try{
    const { name, height, weight, lifeSpan, createdInDb, temperament, image } = req.body;
    ...
    //let temperamentDb = await Temperament.finAll({
